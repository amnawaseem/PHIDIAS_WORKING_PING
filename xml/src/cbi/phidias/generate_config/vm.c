...
const char *capability_type_string(const char *cap_type) {
  ...
  else if (strcmp(cap_type, "start") == 0)
    return "CAPABILITY_TYPE_START_DOMU";
  return NULL;
}
  ...
  iterate_over_children(sctx->doc, "/scenario", "guest", guest) //{
    ...
    for (gcpu = 0; gcpu < cpumap_len; gcpu++) {
      ...
      def_buffer_printf(&def_mem->identifier, "vm_%s_cpu%d_memareas[%d]",
			gid, gcpu, map_count);
      generate_body_memareas(&def_mem->initializer, sctx, A, guest_as, map_count, hcpu,
      ...
      generate_memtree(sctx, A, guest_as, map_count, hcpu, db[0].buf, db[1].buf, NULL);
